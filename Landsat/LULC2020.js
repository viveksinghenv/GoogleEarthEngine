...
var geometry = ee.FeatureCollection("projects/landsat-325606/assets/DMC_Buffer_Shp"),
    farms = ee.FeatureCollection("projects/landsat-325606/assets/LULC_Dhanbad_2020"),
    farms1 = ee.FeatureCollection("projects/landsat-325606/assets/LULC2020");
...
var S2 = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2')
    ...
    .filterDate('2019-04-01', '2020-3-30')
    .map(maskClouds)
...
var meanimage = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2')
    ...
    .select(['SR_B2','SR_B3','SR_B4','SR_B5','SR_B6','SR_B7'],['blue','green','red','nir','swir1','swir2'])
    .map(scaleC2).median();
Map.addLayer(S2,vizParams,'L8_FCC')
// // landsat 5
...
var bandames = S2.bandNames();
print(bandames,'NDVI')
// Map.addLayer(image, visParams, 'L8 NDVI')
var merged = meanimage.addBands(scaledNDVI)
// var collec = meanimage.merge(scaledNDVI)/*.merge(L5coll)*/
//     .sort('system:time_start',true)
//     .map(addNDVI)
// print(collec)
print(merged,'merge')
var bandNames1 = merged.bandNames();
print(bandNames1,'NDVI_New')
// Map.addLayer(collec,vizParams,'L8_with_NDVI')
