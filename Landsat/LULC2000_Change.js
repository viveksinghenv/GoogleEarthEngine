...
var geometry = 
    ...
    ee.Geometry.Polygon(
        [[[86.07697240824143, 24.24291271826135],
          [86.07697240824143, 23.573760086442366],
          [87.11518041605393, 23.573760086442366],
          [87.11518041605393, 24.24291271826135]]], null, false),
    gaul = ee.FeatureCollection("FAO/GAUL_SIMPLIFIED_500m/2015/level2"),
    farms1 = ee.FeatureCollection("projects/landsat-325606/assets/LULC2000_6may"),
    farms2 = ee.FeatureCollection("projects/landsat-325606/assets/LULC2000_6May_n1");
/***** End of imports. If edited, may not auto-convert in the playground. *****/
var dhn = gaul.filter(ee.Filter.eq('ADM2_NAME','Dhanbad'));
var DEM = ee.Image("CGIAR/SRTM90_V4")
var slope =  ee.Terrain.slope(DEM);
// Map.addLayer(slope)
var classNames = ee.FeatureCollection(farms2).select('ORIG_FID')
...
var April = ee.ImageCollection("LANDSAT/LT05/C02/T1_L2")
    .filterBounds(geometry)
    .filterDate('2000-04-01', '2000-04-30')
...
var June = ee.ImageCollection("LANDSAT/LT05/C02/T1_L2")
    .filterBounds(geometry)
    .filterDate('2000-06-01', '2000-06-30')
    .select(['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7'],['blue','green','red','nir','swir1','swir2'])
    .map(scaleC2).mean();
// print (June,'June')
// Function to calculate and add an NDVI band
...
var July = ee.ImageCollection("LANDSAT/LT05/C02/T1_L2")
    .filterBounds(geometry)
    .filterDate('2000-07-01', '2000-07-30')
    .select(['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7'],['blue','green','red','nir','swir1','swir2'])
    .map(scaleC2).mean();
// print (July,'July')
// Function to calculate and add an NDVI band
...
var August = ee.ImageCollection("LANDSAT/LT05/C02/T1_L2")
    .filterBounds(geometry)
    .filterDate('2000-08-01', '2000-08-30')
    .select(['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7'],['blue','green','red','nir','swir1','swir2'])
    .map(scaleC2).mean();
// print (August,'August')
// Function to calculate and add an NDVI band
...
var September = ee.ImageCollection("LANDSAT/LT05/C02/T1_L2")
    .filterBounds(geometry)
    .filterDate('2000-09-01', '2000-09-30')
    // .map(maskClouds)
    .select(['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7'],['blue','green','red','nir','swir1','swir2'])
    .map(scaleC2).mean();
...
var December = ee.ImageCollection("LANDSAT/LT05/C02/T1_L2")
    .filterBounds(geometry)
    .filterDate('2000-12-01', '2000-12-30')
    // .map(maskClouds)
    .select(['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7'],['blue','green','red','nir','swir1','swir2'])
    .map(scaleC2).median();
...
var January = ee.ImageCollection("LANDSAT/LT05/C02/T1_L2")
    .filterBounds(geometry)
    .filterDate('2001-01-01', '2001-01-30')
    // .map(maskClouds)
    .select(['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7'],['blue','green','red','nir','swir1','swir2'])
    .map(scaleC2).mean();
...
var February = ee.ImageCollection("LANDSAT/LT05/C02/T1_L2")
    .filterBounds(geometry)
    .filterDate('2001-02-01', '2001-02-28')
    // .map(maskClouds)
    .select(['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7'],['blue','green','red','nir','swir1','swir2'])
    .map(scaleC2).mean();
...
var March = ee.ImageCollection("LANDSAT/LT05/C02/T1_L2")
    .filterBounds(geometry)
    .filterDate('2001-03-01', '2001-03-31')
    //* .map(maskClouds)*//
    .select(['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B7'],['blue','green','red','nir','swir1','swir2'])
    .map(scaleC2).mean();
// print (March,'March')
...
print(sample,'Sample')
var traininggcp = sample.filter(ee.Filter.lt('random', 0.80))// //70% training and 30% validation
var validation = sample.filter(ee.Filter.gte('random', 0.80));
...
Map.addLayer(postclassify,{min: 0, max: 5,palette: ['0cffff','27a910','81937e','d61717','f1fb5b','0cff08','cf1ad2'/*,'1EEAF9','#d7ffd2'*/]},
'Post_classification')
/******/
...
Export.table.toDrive({
  collection: fc,
  description: 'Accuracy_Export2000',
  folder: 'Change_Interpretation',
  fileNamePrefix: 'accuracy2000_n',
  fileFormat: 'CSV'
})
Export.image.toDrive({
  image: classified,
  description: 'Landsat_2000_n',
  folder: 'Change_Interpretation',
  scale: 30,
  maxPixels: 1e9,
  region: dhn
